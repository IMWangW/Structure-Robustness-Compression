...
class Simulation:
    def __init__(self, Model, TrafficGen, sumo_cmd, max_steps, green_duration, yellow_duration, num_states_width,num_states_height, num_actions):
        ...
        self.every_hour_attack_count = []
        self.every_hour_attack_success_count = []
        self.trigger = True
    ...
    def run(self, episode):
        ...
        self._step = 0
        self.last_attack_all_count = 0
        self.last_attack_success_count = 0
        self._waiting_times = {}
        ...
        while self._step < self._max_steps:
            ...
            if  self._step > (3600*hour_count):
                self.every_hour_attack_count.append(sum(self.all_count_list) - self.last_attack_all_count)
                self.last_attack_all_count = sum(self.all_count_list)
                self.every_hour_attack_success_count.append(sum(self.success_count_list) - self.last_attack_success_count)
                self.last_attack_success_count = sum(self.success_count_list)
                hour_count += 1
            self.calculate_car_local(current_state)
            ...
            print("every action Q value is-->", self._Model.predict_one(current_state))
        ...
        for i in range(24):
            print("self.every_hour_queue_length[{}] = {}\n".format(i, self.every_hour_queue_length[i])) 
        ...
        print("--average_queue_length = ",self.average_queue_length)
        print("Average wait times:", self.average_wait_time)
        print("Total reward:", np.sum(self._reward_episode))
        print("attack record:",hour_count)
