...
if __name__ == "__main__":
    ...
    for i in range(1, prune_episode + 1):
        ...
        from pruning_simulation import Simulation
        from generator import TrafficGenerator
        from memory import Memory
        from utils import import_prune_configuration, set_sumo
        from LRP_neuron_pruner import LRP
        import tensorflow as tf
        ...
        def _load_my_model(model_folder_path, model_folder):
            ...
            if model_folder_path == 'prune_model/model_0':
                loaded_model = load_model('Stu_model/model_2/trained_model.h5')
            else:
                if model_folder == 'prune_model.h5':
        ...
        config = import_prune_configuration(config_file='pruning_settings.ini')
        ...
        path = 'prune_model/model_' + str(i)
        ori_model_folder = 'ft_prune_model.h5'
        if i == 1:
            ori_model_folder = 'trained_model.h5'
            Model = _load_my_model('prune_model/model_' + str(i - 1), ori_model_folder)
        else:
            Model = _load_my_model('prune_model/model_' + str(i - 1), ori_model_folder)
        ...
        model_for_finetune = load_model('prune_model/model_' + str(i) + '/prune_model.h5')
        ...
        save_ft_model_path = 'prune_model/model_' + str(i)
        model_for_finetune.save(os.path.join(save_ft_model_path, 'ft_prune_model.h5'))
        try:
