...
class Simulation:
    def __init__(self, Model, Memory, TrafficGen, sumo_cmd, gamma, max_steps, green_duration, yellow_duration,
                 num_states_width, num_states_height, num_actions, training_epochs, weights_mask):
        self._Model = Model
        self._Memory = Memory
    ...
    def run(self, episode, epsilon, pruned_neuron_location):
        ...
        path = 'attacked-prune_model/model_' + str(episode + 1)
    ...
    def every_hour_wait_times(self):
        ...
        for i in range(24):
            ...
            if every_hour_leave_car_number[i] != 0:
                ...
                # self.average_wait_time[i].append(0.0)
    ...
    def apply_mask_gradients(self, grads_and_vars):
        self.weights_mask = np.array(self.weights_mask)
        masked_gradients_and_vars = [(grad * mask, var) for (grad, var), mask in zip(grads_and_vars, self.weights_mask)]
        
        return masked_gradients_and_vars
    ...
    def _replay(self):
        ...
        if len(batch) > 0:  # if the memory is full enough
            ...
            # model_weights = self._Model.get_weights()
            # bbb = model_weights[0][268][42]
            # # bbb = tf.convert_to_tensor(bbb)
            # ccc = model_weights[0][88][42]
            # # ccc = tf.convert_to_tensor(ccc)
            # # bbb = tf.stop_gradient(bbb)
            # # ccc = tf.stop_gradient(ccc)
            # a = 0

            with tf.GradientTape() as tape:
                # prediction
                q_s_a = self._Model(states)  # predict Q(state), for every sample
                q_s_a_d = self._Model(next_states)  # predict Q(next_state), for every sample
            ...
            masked_gradients = self.apply_mask_gradients(zip(grads, self._Model.trainable_variables))
            opt = tf.keras.optimizers.Adam(learning_rate=0.001)
            opt.apply_gradients(masked_gradients)
