...
class LRP:
    ...
    def lrp_R(self):
        ...
        for i in range(len(self.state)):
            ...
            # lrp_result = np.reshape(lrp_result,(8,10))
            # plt.imshow(lrp_result, cmap='jet')
            # plt.show()
        ...
        # print(value_k)
        # print(index_k)
        weights_mask = self.non_main_neuron_prune(index_k)
        print("==============================================")
        print("==============================================")
    ...
    def non_main_neuron_prune(self, index_k):
        ...
        new_weights = copy.deepcopy(model_weights)
        record_row = []
        record_col = []
        ...
        which__ = 0
        weights_array = [np.array(w) for w in new_weights]
        weights_mask = [np.ones_like(w) for w in weights_array]
        for i, w in enumerate(weights_mask):
            print("Layer", i, "weights array shape:", w.shape)
            """
            Layer 0 weights array shape: (800, 800)
            Layer 1 weights array shape: (800,)
            Layer 2 weights array shape: (800, 300)
            Layer 3 weights array shape: (300,)
            Layer 4 weights array shape: (300, 400)
            Layer 5 weights array shape: (400,)
            Layer 6 weights array shape: (400, 4)
            Layer 7 weights array shape: (4,)
            <class 'list'>
            """
        ...
        for m, n in zip(record_row, record_col):
            for r in prune_index[which__]:
                new_weights[m][r][n] = 0 
                weights_mask[m][r][n] = 0
            which__ += 1

        self.model.set_weights(new_weights)
        path = 'attacked-prune_mask-ft_model/model_' + str(self.prune_count)
        self.model.save(os.path.join(path, 'attacked-prune_model.h5'))
